...
class BibliometricNetworkAnalyzer:
    ...
    def extract_co_citation_network(self, min_weight: int = 1) -> nx.Graph:
        ...
        # Add nodes (papers)
        for record in self.graph.run(paper_query):
        ...
        for record in self.graph.run(cocitation_query, min_weight=min_weight):
    ...
    def extract_author_collaboration_network(self) -> nx.Graph:
        ...
        # Add nodes (authors)
        for record in self.graph.run(author_query):
            G.add_node(
                record['name'],
                orcid=record['orcid']
            )
        ...
        for record in self.graph.run(collaboration_query):
            G.add_edge(
                record['source'],
                record['target'],
                weight=record['weight']
    ...
    def extract_institution_collaboration_network(self) -> nx.Graph:
        ...
        for record in self.graph.run(institution_query):
    ...
    def extract_keyword_co_occurrence_network(self) -> nx.Graph:
        ...
        # Add nodes (keywords)
        for record in self.graph.run(keyword_query):
            G.add_node(record['name'])
        
        # Add edges (co-occurrence relationships)
        for record in self.graph.run(cooccurrence_query):
            G.add_edge(
                record['source'],
                record['target'],
                weight=record['weight']
