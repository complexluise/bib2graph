...
vía ``run_build`` (pasada cited_by automática con seeds aceptadas), verificando
el comportamiento end-to-end que antes no podía validarse porque ``cited_by_id``
quedaba vacío en uso real.
...
que ejercían estos tests vive ahora en ``run_build`` — mismo camino de
persistencia (dispara automáticamente cuando hay seeds aceptadas, ADR 0038
§enrich), fuente única.

Casos cubiertos:
1. ``cited_by_id`` se popula en las seeds aceptadas tras ``run_build`` con
...
3. Idempotencia de la pasada cited_by: re-correr ``run_build`` sobre el mismo
   store produce el mismo estado (mismo ``corpus_hash`` y misma red cocitation).
4. ``max_citing`` acota el fetch: con tope = 1, ``cited_by_id`` tiene ≤1
   citante por seed; el estado del store es consistente.
5. Corpus chico sin seeds aceptadas → ``run_build`` termina sin error y sin
   alterar el ``cited_by_id`` del corpus (0 citantes).
...
def test_run_enrich_puebla_cited_by_en_seeds_aceptadas(tmp_path: Path) -> None:
    """run_build (pasada cited_by) con transport mockeado puebla cited_by_id.

    Escenario: 2 seeds aceptadas (W100, W200) + 1 citante compartido C1 que
    cita a ambas. Tras run_build, cited_by_id debe contener C1 en ambos papers.

    Verifica la integración end-to-end: store → run_build → store → load.
    """
    ...
    data = run_build(store_path, out_dir=tmp_path / "networks", transport=transport)
    enrichment = data["enrichment"]

    # Verificar métricas de la función
    assert enrichment["citing_targets"] == 2, (
        f"Se esperaban 2 seeds objetivo, se obtuvieron {enrichment['citing_targets']}"
    )
    assert enrichment["citing_new"] >= 1, (
        f"Se esperaba ≥1 citante nuevo, se obtuvieron {enrichment['citing_new']}"
    )
    ...
    assert "C1" in (rows_out["W100"].get("cited_by_id") or []), (
        "W100 debe tener C1 en cited_by_id tras run_build"
    )
    assert "C1" in (rows_out["W200"].get("cited_by_id") or []), (
        "W200 debe tener C1 en cited_by_id tras run_build"
    )
...
def test_run_enrich_produce_red_cocitacion_con_arista_esperada(tmp_path: Path) -> None:
    """Tras run_build (pasada cited_by), Networks.quick incluye cocitation con ≥1 arista.
    ...
    citing_works = [_citing_work("C1", ["W100", "W200"])]
    transport = _make_cited_by_transport(citing_works)
    run_build(store_path, out_dir=tmp_path / "networks", transport=transport)
...
def test_run_enrich_idempotente_corpus_hash_y_cocitacion(tmp_path: Path) -> None:
    """Re-correr run_build (cited_by) produce el mismo corpus_hash y la misma cocitation.

    Verifica que:
    (a) El corpus_hash es idéntico antes y después de un segundo run_build.
    ...
    run_build(store_path, out_dir=tmp_path / "networks2", transport=transport2)
    ...
    assert hash_after_1 == hash_after_2, (
        ...
        "El enrich (via run_build) debe ser idempotente (mismo corpus → mismo hash)."
    )
...
def test_run_enrich_max_citing_acota_cited_by_id(tmp_path: Path) -> None:
    """run_build con max_citing=1 acota cited_by_id a ≤1 citante por seed.

    Verifica que el tope ``max_citing`` propagado a ``run_build`` se respeta
    ...
    run_build(
        store_path, out_dir=tmp_path / "networks", transport=transport, max_citing=1
    )
...
def test_run_enrich_sin_seeds_aceptadas_no_altera_corpus(tmp_path: Path) -> None:
    """Corpus sin seeds aceptadas → run_build no dispara la pasada cited_by.

    Verifica que el pipeline no falla y que el corpus_hash del corpus (sin
    contar redes/artefactos) permanece igual cuando no hay targets válidos
    para el enrich de co-citación (ADR 0038 §enrich: no-op graceful).
    """
    from bib2graph.cli.commands.build import run_build
    ...
    assert enrichment == {}, (
        f"Sin seeds aceptadas, enrichment debe ser vacío (no-op graceful): {enrichment}"
    )
    ...
    assert hash_before == hash_after, (
        ...
        "run_build no debe alterar el corpus cuando no hay targets de cited_by."
    )
