...
| [0043](0043-precedencia-inclusion-manual-en-curate.md) | Precedencia en `curate filter`: la **inclusión manual gana** — el filtro PRISMA **nunca** mueve un paper `accepted`→`rejected` (actúa solo sobre no-aceptados); cierra el *footgun invisible* de reclasificar sin aviso decisiones humanas | **Aceptada** (2026-06-30). **Precisa [0020](0020-metodo-forrajeo-scent-filtros-reject.md) §C** (acota el scope del filtro que marca `rejected`, sin revertirlo). Relacionada 0016 (curación transversal = lo irreductiblemente humano)/0037 (D2: `curate filter`→`FILTERED`)/0009/0013/0022. Origen: issue [#233](https://github.com/complexluise/bib2graph/issues/233); futuro `--force` mencionado, no decidido |
